...
function Movies(){
  ...
  function xyz(){
    ...
    return(
      ...
      {/* left bar */}
          <div class=" w-full lg:w-1/12 bg-slate-900  lg:h-screen z-50">
            {/* nav bar */}
          ...
          {/* right bar */}
         <div class="w-full lg:w-11/12 bg-slate-900 h-screen ">
          {/* vertical grid */}
               <div class="lg:h-20 xl:h-24 md:pt-0 lg:pt-2 xl:pt-0 bg-slate-900 w-full flex items-center fixed xl:sticky top-16 md:top-20 lg:top-0 z-50">
                {/* search bar */}
                <div className="w-full  bg-slate-900 flex justify-center items-center pl-8 md:px-12 lg:px-4 h-16 rounded-lg xl:pt-4 lg:mr-24 xl:mr-2"><CiSearch className="text-white text-2xl stroke-2 "/>
                     <input placeholder="Search for Movies or TV Series" class="bg-slate-900 text-gray-200 border-0 rounded-md p-2 px-4 border-none focus:outline-none  transition ease-in-out duration-150 w-full" type="text" value={searchItem} onChange={handleSearchChange} name="search" />
                     <button   onClick={xyz} className=' text-gray-200 p-2 px-4 md:px-6 xl:px-8 rounded-md xl:rounded-lg xl:mr-6 bg-slate-800 '>search
                       </button>
                     </div>
               </div>
               <div class=" bg-slate-900 mt-28 pt-4 md:pt-0 md:mt-36 lg:mt-20 xl:mt-0">
                {/* trending card */}
                <div className='text-white text-2xl ml-5 md:pl-2 lg:pl-0'>Movies</div>
                <div className='flex  flex-wrap flex-start justify-between'>{renderMovies}</div>
               </div>
         </div>
    </div>
    )
