...
function TrendingCard(props){
  ...
  async  function handleevent(data,e){
    ...
    alert("working")
    try {
      const response = await axios.post(bookmarkURL,data, {
     ...
     let link = (data.name)?(`/tvcast/${data.id}`):(`/cast/${data.id}`) 
      return(
        ...
        <div className="mx-4 md:mx-3 lg:mx-6 mt-3 bg-slate-900  rounded-xl hover:bg-gray-900 hover:scale-110 duration-700 h- ">
          <div style={{ backgroundImage: `url(https://image.tmdb.org/t/p/w500${data.poster_path})` }} className="rounded bg-cover bg-no-repeat bg-top h-40 w-96">
          <div className="h-8 w-8 rounded-full float-right pt-2 mt-2 mr-8  bg-black opacity-50 hover:bg-gray-400 text-center"><button type="button" className="" onClick={(e)=>{handleevent(data,e)}}> 
